...
_start:
    ...
    mov     $boot_pml4, %eax
    or      $(PTE_ATTR_P | PTE_ATTR_RW), %eax
    mov     %eax, boot_pml4

    mov     $boot_pdpt, %eax
    or      $(PTE_ATTR_P | PTE_ATTR_RW), %eax
    mov     %eax, (boot_pml4 + 0x0)

    mov     $boot_pd, %eax
    or      $(PTE_ATTR_P | PTE_ATTR_RW), %eax
    mov     %eax, (boot_pdpt + 0x0)
    ...
    mov     $0x0, %eax
    or      $(PTE_ATTR_P | PTE_ATTR_RW | PDE_ATTR_PS), %eax
    mov     %eax, (boot_pt + 0x0)
...
long_mode_entry:
    ...
    push    %rsi  # Multiboot2 information structure
...
.section .data
    ...
    .globl boot_stack, boot_stack_top, boot_pml4, boot_pdpt, boot_pd, boot_pt
